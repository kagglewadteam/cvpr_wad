...
class MaskRCNN():
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,           #the whole method is only for one image
                          image_shape, window):
        ...
        N = zero_ix[0] if zero_ix.shape[0] > 0 else detections.shape[0]     #this stands for no detection

        # Extract boxes, class_ids, scores, and class-specific masks, those are all numpy arrays
        boxes = detections[:N, :4]
        class_ids = detections[:N, 4].astype(np.int32)
        scores = detections[:N, 5]
        masks = mrcnn_mask[np.arange(N), :, :, class_ids] #the mask channel is arranged by its class id
        ...
        for i in range(N):
            ...
            full_mask = utils.unmold_mask(masks[i], boxes[i], original_image_shape)     #Question: masks[i] should be a 3D tensor, or due to delete it must be a 2D tensor?
    ...
    def detect(self, images_set, verbose=0):
        ...
        """
        images = zip(*images_set)[0]
        image_ids = zip(*images_set)[1]
