...
def dataset_split(data_dir, label_dir, valid_train_rate = 0.1, shuffle = True, seed = None):
    ...
    validation_num = int(valid_train_rate * len(data_list))
    print('validation_num', validation_num)
    validation_path = [data_list[i] for i in range(validation_num)]
    train_patient_num = len(data_list) - validation_num
    print('train_num', train_patient_num)
    train_path = [data_list[validation_num + i] for i in range(train_patient_num)]
